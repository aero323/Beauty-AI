    ...
    <div className="flex-1 flex flex-col pt-2 h-[calc(100vh-5rem)] overflow-y-auto">
      ...
      <div className="space-y-6 max-w-4xl">
        {channels.map((channel) => (
          channel.id === 'whatsapp' ? (
            <div key={channel.id} className="relative group/dev-note">
              ...
              <Card className={`rounded-2xl shadow-sm border transition-colors ${channel.enabled ? 'border-rose-200 bg-white' : 'border-[#E9E4DF] bg-[#F8F5F3] opacity-80'}`}>
              ...
              <div className="absolute right-0 bottom-full mb-2 hidden group-hover/dev-note:block z-[80] w-72 rounded-lg bg-blue-950/95 px-3 py-2 text-xs leading-relaxed text-white shadow-xl backdrop-blur-sm">
                研发备注：其他渠道沟通配置，一期不做，二期不做。
              </div>
            </div>
          ) : (
