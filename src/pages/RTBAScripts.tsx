    ...
    <div className="flex h-full bg-[#FAF9F8] overflow-hidden pt-2 rounded-xl border border-slate-200">
      ...
      <div className="w-80 bg-white border-r border-slate-200 flex flex-col shrink-0">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between z-10 bg-white">
          <h2 className="font-bold text-slate-800 tracking-tight">区域场景剧本 ({scripts.length})</h2>
          <button 
            onClick={handleAddNew}
            className="flex items-center justify-center p-1.5 rounded-md bg-amber-50 text-amber-600 hover:bg-amber-100 transition-colors"
          >
            ...
            >
              <div className="flex justify-between items-start">
                <div className="flex items-start space-x-3 w-full">
                  <div className={`p-2 rounded-lg shrink-0 mt-0.5 ${selectedId === script.id ? 'bg-amber-100 text-amber-600' : 'bg-white text-slate-400'}`}>
                    <FileText className="h-5 w-5" />
                  </div>
                  <div className="min-w-0 pr-6 w-full">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className={`font-bold text-sm truncate ${selectedId === script.id ? 'text-amber-900' : 'text-slate-800'}`}>
                        {script.name}
                      </h3>
                    ...
                    <p className={`text-xs mt-1 line-clamp-2 ${selectedId === script.id ? 'text-amber-700/70' : 'text-slate-500'}`}>
                       {script.description}
                    </p>
                  </div>
                </div>
        ...
        {selectedScript ? (
          <>
            <div className="p-6 border-b border-slate-200 bg-white flex items-center justify-between shrink-0">
              <div>
                <h1 className="text-xl font-bold text-slate-800">编辑区域剧本：{selectedScript.name}</h1>
                <p className="text-xs text-slate-500 mt-1">配置剧情背景与区域适用的考核指导节点</p>
              </div>
              <button 
                onClick={handleSave}
                className="flex items-center space-x-2 px-5 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg shadow-sm font-bold text-sm transition-colors"
              >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input 
                        ...
                        className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-shadow font-medium"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-500 mb-2">区域场景介绍</label>
                      <textarea 
                        value={selectedScript.description}
                        onChange={(e) => handleUpdate('description', e.target.value)}
                        rows={3}
                        className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-shadow resize-none"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                  ...
                  <div className="flex border border-slate-200 rounded-xl overflow-hidden bg-slate-50/50">
                    ...
                    <div className="flex-1 p-6 flex flex-col justify-center space-y-4">
                      <div>
                        <p className="text-sm font-medium text-slate-700">可以上传一张图，来生动展示区域门店场景</p>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <div className="flex items-center gap-2">
                      <Button onClick={handleGenerateScriptSteps} variant="secondary" size="sm" className="h-8 bg-amber-50 text-amber-700 hover:bg-amber-100 hover:text-amber-800 border-none shadow-sm">
                        <Wand2 className="h-4 w-4 mr-1" />
                        AI 一键生成剧本
                      </Button>
                      <Button onClick={handleAddStep} variant="outline" size="sm" className="h-8 shadow-sm border-amber-200 text-amber-600 hover:bg-amber-50 hover:text-amber-700">
                  ...
                  <div className="space-y-4">
                    {selectedScript.steps.map((step, index) => (
                      <div key={step.id} className="p-4 rounded-xl border border-slate-200 bg-slate-50/50 relative group">
                        <div className="absolute left-4 top-4 bg-amber-100 text-amber-700 font-bold text-xs h-6 w-6 flex items-center justify-center rounded-md">
                          {index + 1}
                        </div>
                        <div className="pl-10 space-y-3">
                          ...
                          <div>
                            <label className="block text-xs font-bold text-slate-500 mb-1 flex items-center">
                              <AlertCircle className="h-3 w-3 mr-1 text-slate-400" />
                            ...
                            <textarea 
                              ...
                              rows={2}
                              className="w-full px-3 py-1.5 border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-slate-500/20 focus:border-slate-500 bg-white resize-none"
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
            <FileText className="h-16 w-16 mb-4 opacity-20 text-amber-500" />
            <p className="font-medium text-slate-500">在左侧选择或创建一个区域剧本</p>
          </div>
        )}
      </div>
    </div>
