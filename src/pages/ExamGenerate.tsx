...
import { UploadCloud, FileText, CheckCircle, Loader2, Database, AlertCircle, Trash2, Wand2 } from 'lucide-react';
          ...
          <CardContent className="flex flex-col items-center justify-center py-16 px-4 text-center">
            ...
            <div className="flex space-x-4 mb-8">
              <div className="flex items-center px-4 py-2 bg-[#F8F5F3] rounded-lg border border-[#E5DED8] text-sm">
                ...
                <span className="font-medium text-[#3F3A3D]">Lumina_新品精华_培训版.pdf</span>
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[#766F73] mt-1">共生成 3 道题目，请审核并导入题库。在生成课件时如选择同步生成题目，也会在此统一查收进行入库操作。</p>
            </div>
            <div className="flex space-x-3">
              <Button variant="outline" onClick={() => setStep('upload')}>返回重新生成</Button>
              <Button className="bg-rose-600 hover:bg-rose-700 text-white" onClick={handleImport}>
          ...
          <div className="grid gap-4">
            {mockQuestions.map((q, i) => (
              <Card key={q.id}>
                <CardHeader className="pb-3 border-b border-[#E9E4DF] flex flex-row items-center justify-between space-y-0">
                  <div className="flex items-center space-x-3">
                    <Badge variant="outline" className="bg-rose-50 text-rose-700 border-rose-200">
                      {q.type === 'SingleChoice' ? '单选题' : q.type === 'MultipleChoice' ? '多选题' : '判断题'}
                    </Badge>
                    <span className="text-xs text-[#766F73]">来源: Lumina_新品精华_培训版.pdf</span>
                  </div>
                ...
                <CardContent className="pt-4 space-y-4">
                  <div className="font-bold text-[#242124] text-sm">
                    {i + 1}. {q.content}
                  </div>
                  <div className="space-y-2">
                    {q.options.map(opt => (
                      <div key={opt} className={`p-2 rounded border text-sm ${q.answer.includes(opt.charAt(0)) || opt === q.answer ? 'bg-[#EEF8F4] border-emerald-200 text-emerald-800 font-medium' : 'bg-[#F8F5F3] border-[#E5DED8] text-[#5D565A]'}`}>
                        {opt}
                      </div>
                    ))}
                  </div>
                  <div className="text-sm font-medium text-[#3B8F72] flex items-center">
